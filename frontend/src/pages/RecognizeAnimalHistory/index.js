...
function RecognizeAnimalHistory() {
      ...
      .then((response) => {
        if (response.data !== null) {
          ...
          response.data.map((item) => {
            console.log(item.animalScientificName);
            axios
              .get("http://localhost:8080/api/v1/animals/" + item.animalScientificName)
              .then((responseDetail) => {
                if (responseDetail.data.status === "ok") {
                  console.log(responseDetail.data);
                  setNameAnimal((pre) => [...pre, responseDetail.data.data.animalName]);
                }
              })
            ...
            axios
              .get(
                "http://localhost:8080/api/v1/animals/detailbyanimalscientificname/" +
                  item.animalScientificName
              )
              .then((responseDetail) => {
                if (responseDetail.data !== null) {
                  setLocationAnimal((pre) => [...pre, responseDetail.data.countries]);
                }
              })
              .catch((error) => console.log(error));
          });
  ...
  const showDetail = (animalScientificNameAnimal) => {
    console.log(animalScientificNameAnimal);
    setShowDetailAnimal(true)
    setAnimalScientificName(animalScientificNameAnimal);
  };
                ...
                <div
                  ...
                  onClick={(event) => {
                    if (!event.target.classList.contains(cx("delete_but"))) {
                      showDetail(item.animalScientificName);
                    }
                  }}
                >
                  ...
                  <div>
                    ...
                    <div className={cx("information")}>
                      <span className={cx("location")}>{locationAnimal[index]}</span>
                    </div>
                    <div className={cx("information")}>
                      <button
