...
function PostAnimal() {
  ...
  const handleFormDetailClick = (index, e) => {
    if (AnimalIndexesWithEmptyAttributes.includes(index)) {
      setAnimalIndexesWithEmptyAttributes(
        AnimalIndexesWithEmptyAttributes.filter((item) => item !== index)
      );
      return;
    }
    if (
  ...
  const handleCombine = () => {
    ...
    if (allSame) {
      ...
      tempAnimal.forEach((animal) => {
        selectedFiles = [
          ...selectedFiles,
          ...(Array.isArray(animal.files) ? animal.files : [animal.files]),
        ];
        selectedPreviews = [
          ...selectedPreviews,
            ...
            <div onClick={handleButtonClick}>
              {isDragActive ? (
                ...
                <p className={cx("introduction")}>
                  Kéo và thả hình ảnh vào đây hoặc nhấn để chọn ảnh
                </p>
              )}
            </div>
          </div>
