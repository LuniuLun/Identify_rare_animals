...
function Header() {
      ...
      axios
        ...
        .then(async (resRecognize) => {
          ...
          if (resRecognize.status === 200) {
            ...
            const confidenceWithoutPercentage = parseInt(
              confidenceWithPercentage.replace(/%/g, "")
            ); // 74
            if (confidenceWithoutPercentage < 60) {
        ...
        .catch((error) => {
          ...
          setShowWarning(true);
          setContentWarning("Lỗi mạng!!! Không thể gửi ảnh đi, vui lòng kiểm tra lại mạng hoặc F5 lại trang");
        });
    }
  ...
  const saveRecognitionResult = async () => {
    ...
    const saveRecognition = await axios.post(
      "http://localhost:8080/api/v1/users/newResult",
      requestData,
      {
        headers: {
      ...
      <div className={cx("right-item")}>
        ...
        <div className={cx("list-options")}>
          ...
          {role === "1" ? (
            <Link
              className={cx("link")}
              to={"/your_observation/" + sessionStorage.getItem("userID")}
            >
          ...
          <div className={cx("modal-content")}>
            ...
            {imageUrl !== "" ? (
              ...
              <>
                  ...
                  <div className={cx("detail-result")}>
                    ...
                    <div className={cx("list-names")}>
                      ...
                      <div className={cx("name")}>
                        ...
                        <input className={cx("scientific-name")} value={scientificName} readOnly />
                      </div>
                      <div className={cx("name")}>
                ...
                <div className={cx("wrapper-btn_save-result")}>
                  <button className={cx("save-recognization-result")} onClick={reloadPage}>
                    Nhận diện khác
                  </button>
                </div>
              </>
